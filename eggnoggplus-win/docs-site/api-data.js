...
mod.ui.end_overlay()`, { tags: ["cosmetic"], notes: ["Balance each successful begin_overlay with end_overlay in the same callback."] }),
  ...
  scale=2, tint={1, 0.8, 0.7, 1}
    ...
    fn("mod.ui.navigate_items", "mod.ui.navigate_items(items, selected, direction [, options])", "Moves selection to the previous or next enabled entry without polling input.", [p("items", "table", "Dense array of up to 4096 items; id/value/index keys and optional disabled flag."), p("selected", "string | number | nil", "Current selection key."), p("direction", "integer", "-1 previous, 0 unchanged, 1 next."), p("options", "table", "wrap=false, disabled=false.", "optional")], "Selection key and changed flag.", `selected, changed = mod.ui.navigate_items(items, selected, 1, {wrap=true})`, { tags: ["read-only"], notes: ["API revision 8 / ui.navigation. Empty or all-disabled lists retain selection. tabs, item_grid and swatch_grid also accept navigate, wrap and disabled options. Callers route keyboard/controller edges to the focused widget."] }),
    fn("mod.ui.grid_layout", "mod.ui.grid_layout(x, y, width, count [, options])", "Calculates responsive equal-width grid cells without changing the UI cursor.", [p("x, y", "number", "Finite origin within +/-10,000,000."), p("width", "number", "Available positive width, up to 10,000,000."), p("count", "integer", "Number of cells, 0 through 4096."), p("options", "table", "min_cell_w=120, cell_h=28, gap=6, max_cols=4096. Dimensions are positive, gap nonnegative; max_cols is an integer 1 through 4096.", "optional")], "Detached {x,y,w,h} array, total height, column count. Empty grids return {}, 0, 0.", `local cells, height, columns = mod.ui.grid_layout(40, 80, width, #items, { max_cols=4 })`, { tags: ["read-only"], notes: ["Requires API revision 7 / ui.responsive_grid. A narrow container uses one column; the last row keeps the regular cell width."] }),
    fn("mod.ui.grid_layout", "mod.ui.grid_layout(x, y, width, count [, options])", "Calculates responsive equal-width grid cells without moving the UI cursor.", [p("x, y", "number", "Finite origin within +/-10,000,000."), p("width", "number", "Positive width up to 10,000,000."), p("count", "integer", "Cell count from 0 through 4096."), p("options", "table", "min_cell_w=120, cell_h=28, gap=6, max_cols=4096. Positive dimensions, nonnegative gap, integer max_cols from 1 through 4096.", "optional")], "Detached {x,y,w,h} array, total height, columns. Empty grids return {}, 0, 0.", `local cells, height, columns = mod.ui.grid_layout(40, 80, width, #items, { max_cols=4 })`, { tags: ["read-only"], notes: ["Requires API revision 7 / ui.responsive_grid. Narrow containers use one column; last-row cells retain regular width."] }),
    fn("mod.ui.list_box", "mod.ui.list_box(id, items, selected, opts)", "Draws a filtered, scrollable selectable list with keyboard paging and a pointer scrollbar.", [p("items", "table", "Up to 4096 items using unique id, label, disabled fields."), p("opts", "table", "x/y/w/h/row_h bounds; scroll and scroll_rows in rows; filter; navigate: none/previous/next/page_up/page_down/first/last; focused, activate, reveal, disabled; style/text_scale.")], "selected, changed, scroll, activated, match_count", `selected, changed, scroll, activated = mod.ui.list_box("rooms", rooms, selected, {
  x=16, y=44, w=260, h=168, scroll=scroll, navigate=command, focused=true
})`, { notes: ["API revision 12 / ui.list_box. Only complete visible rows are drawn and hit-tested. Literal case-insensitive filtering preserves hidden selections without activating them. Navigation skips disabled rows and reveals selection. Input edges belong to the caller; scroll is a zero-based row offset. Long labels are shortened to fit. See docs/examples/ui_list_browser.lua for a full demo."] }),
    fn("mod.ui.list_window", "mod.ui.list_window(count, row_h, viewport_h, scroll [, reveal_index])", "Calculates visible fixed-height list rows and scroll limits in constant space.", [p("count", "integer", "0 through 1,000,000."), p("row_h, viewport_h", "number", "Finite positive heights from 1 through 10,000,000."), p("scroll", "number", "Pixel offset, clamped to the list. Finite within +/-1e13."), p("reveal_index", "integer", "Existing one-based row to reveal when selection changes.", "optional")], "Detached {first,last,offset_y,scroll,max_scroll,total_h}; empty lists use first=1,last=0.", `local window = mod.ui.list_window(#items, 28, 280, scroll, selected)
scroll = window.scroll`, { tags: ["read-only"], notes: ["API revision 7 / ui.virtual_list. Includes partial rows; callers own viewport clipping, input routing and focus. Does not allocate per-item bounds."] }),
...
mod.ui.next_row(layout, 28)`, { tags: ["cosmetic"] }),
  ...
  const uiHelpers = [
    ["theme_names", "mod.ui.theme_names()", [], "Sorted detached array of registered theme names.", `for _, name in ipairs(mod.ui.theme_names()) do mod.log(name) end`, "API revision 11 / ui.themes. Includes default_dark and high_contrast; partial named themes inherit default_dark spacing, colors, and text scale."],
    ["with_style", "mod.ui.with_style(style, callback, ...)", [p("style", "table", "Partial style overrides."), p("callback", "function", "Callback; its scope remains isolated if the coroutine yields."), p("...", "any", "Arguments forwarded to the callback.", "optional")], "All callback results, including nil values.", `mod.ui.with_style({pad=12}, function() mod.ui.button("ok", "OK") end)`, "API revision 9 / ui.scoped_style. Restores the previous style stack after success or error, including nested scopes and unbalanced style changes. Rethrows the original error. Scopes are coroutine-local. Named theme registrations persist. Cyclic tables and nesting beyond 32 table levels are rejected."],
...
-- os.exit(2)`, { tags: ["internal"], notes: ["Do not use for ordinary mod errors. mod.error logs safely; returning from a callback keeps the game alive."] })
  ...
  mirror_with_room=true
      ...
      fn("map.on_timer", "map.on_timer(name, callback)", "Declares a rollback-safe named timer during script loading.", [], "No return value.", `map.on_timer("pulse", function() map.state.pulses = (map.state.pulses or 0) + 1 end)
map.timer_start("pulse", 60, 120)`, { notes: ["Map API 12. Up to 32 unique 1-31 byte names without NULs. Lua callbacks cannot capture upvalues. Due timers fire in registration order before on_tick and after contact/leave callbacks."] }),
      fn("map.timer_start", "map.timer_start(name, delay [, interval])", "Starts or replaces a declared timer.", [], "No return value.", `map.timer_start("pulse", 60, 120)`, { notes: ["Delay 1..1000000000 simulation ticks. Interval 0 (default) is one-shot; otherwise the same positive range. Invalid arguments do not mutate the timer. Starting a later due timer from a callback suppresses its old pending firing."] }),
      fn("map.timer_cancel", "map.timer_cancel(name)", "Stops a named timer, including a pending firing later in this dispatch.", [], "Whether the timer was running or pending.", `map.timer_cancel("pulse")`),
      fn("map.timer_remaining", "map.timer_remaining(name)", "Reads ticks remaining on a declared timer.", [], "Remaining simulation ticks, or 0 when stopped/due.", `local remaining = map.timer_remaining("pulse")`),
      fn("map.state_clear", "map.state_clear([prefix])", "Removes rollback-owned state entries with a literal byte prefix.", [], "Number of entries removed.", `map.state_clear("puzzle_")`, { notes: ["Map API version 11. Omit prefix or use an empty string to clear everything. Prefix must be a 0-31 byte string without NULs; explicit nil and extra arguments are errors. Validation precedes mutation. Freed slots can be reused immediately."] }),
      fn("map.state_keys", "map.state_keys()", "Lists existing rollback-owned state keys in bytewise lexical order.", [], "Detached array of up to 64 keys; empty state returns {}.", `for _, key in ipairs(map.state_keys()) do
  map.state[key] = nil
end`, { tags: ["read-only"], notes: ["Map API version 10. Available during loading and callbacks. Insertion order and slot reuse do not change ordering. Read values through map.state[key]; mutating this array cannot change native state."] }),
      fn("map.tile_bindings", "map.tile_bindings()", "Lists declared custom tile bindings in manifest order.", [], "Fresh array of {symbol, key} copies; empty when no bindings exist.", `for _, binding in ipairs(map.tile_bindings()) do\n  map.on_enter(binding.key, function(object, tile)\n    map.state.contacts = (map.state.contacts or 0) + 1\n  end)\nend`, { tags: ["read-only"], notes: ["Map API version 8. Available during loading and callbacks. Mutating returned tables does not change native bindings. Does not enumerate unbound native glyphs or room placements."] }),
  ...
  object.vy = -4
      ...
      fn("tile:get_sprite", "tile:get_sprite()", "Inspects this cell's active temporary sprite override.", [], "Detached {sprite_index, offset_x, offset_y, ticks_left} table, or nil when inactive.", `local visual = tile:get_sprite()\nif visual and visual.ticks_left == 1 then\n  tile:reset_sprite()\nend`, { tags: ["read-only"], notes: ["Map API version 9. Pixel offsets match the quantized override. ticks_left counts script advances to expiry, including the current callback tick; set_sprite(..., 2) initially reports 3. Does not inspect underlying atlas animation. Mutating the table does not change the effect."] }),
...
mod.ui.native_remove("open")`
  ...
  revise("mod.ui.slider", {
    notes: ["API revision 10 / ui.control_navigation: navigate=-1/0/1 changes the value by keyboard_step, positive step, or 1% of the range. Mouse input takes precedence. focused=true draws an outline. Route press edges only to the focused slider."]
  });
  revise("mod.ui.checkbox", {
    notes: ["API revision 10 / ui.control_navigation: focused=true draws an outline; activate=true toggles only while focused and enabled. Pass a press edge, not a held-input flag."]
  });
  revise("mod.ui.tooltip", {
    returns: "true when a non-empty tooltip was drawn; false for empty input."
  });
  revise("mod.ui.icon_button", {
    notes: ["API revision 10 / ui.control_navigation: focused=true draws an outline; activate=true activates only while focused and enabled. Route press edges explicitly."],
  ...
  revise("mod.ui.progress_bar", {
    ...
    params: [
      ...
      p("options", "table", "interactive, disabled, vertical, reverse, fill, rail, border, label, and tooltip settings. Vertical fill and input run bottom-to-top unless reversed. Invalid bounds are ignored.", "optional")
    ],
  ...
  revise("mod.ui.define_state", {
    notes: ["A screen change in a lifecycle callback stops subsequent callbacks and cursor work for the old screen. Redirecting from leave does not repeat the old leave callback."],
    errors: "Invalid lifecycle callback types or failed native registration return false plus an error; an existing definition is preserved.",
