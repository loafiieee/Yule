...
#define GGPO_NET_DEFAULT_MAX_PREDICTION 16
#define GGPO_NET_DEFAULT_MAX_FRAME_ADVANTAGE 12
#define GGPO_NET_DEFAULT_INPUT_DELAY 2
#define GGPO_NET_TIMEOUT_TICKS 600
#define GGPO_NET_CORRECTION_TIMEOUT_TICKS 2400
#define GGPO_NET_MAX_BLOCK_TICKS 15
...
static void ggpo_net_ensure_local_fingerprint(void) {
    ...
    /* Keep this fingerprint static.  State size depends on the current map and
     * thing/tilemap layout, so mixing it here made identical builds look
     * incompatible when peers reached the handshake at different moments. */
...
static int ggpo_net_accept_packet_source(const struct sockaddr_in* from, uint32_t session_id, const char* kind) {
    ...
    /* During UDP hole punching the signaling server may give us one endpoint
     * while the first authenticated game packet arrives from another mapped
     * endpoint.  Before the session is connected and before frame 0 has been
     * loaded, allow that endpoint migration so direct internet play has a
     * chance on more consumer NATs.  Once connected, pin the peer address. */
