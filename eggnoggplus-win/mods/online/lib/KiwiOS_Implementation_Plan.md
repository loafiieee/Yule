...
mkfs.kifs can format a partition and create test files.

Everything runs in ring 0. There are no processes, no syscalls, no ring 3.

## Where We're Going

The kernel boots, mounts KiFS, loads `/bin/init` as a ring 3 process.
Init launches a shell. The shell launches programs. Everything in userspace
communicates with the kernel through syscalls. A preemptive scheduler
time-slices between processes.
...
See the full spec document for field definitions.
...
Phase 14: KiFS write support                    (4-6 hours)
Phase 15: Host-side disk tools                  (3-4 hours)
Phase 16: Cleanup + polish                      (ongoing)
```
