...
-- ── input reading ────────────────────────────────────────────────────────
local function read_raw_slot(player_index)
...
local function read_local_cmd()
    return read_raw_slot(local_idx)
end
...
local function resolve_remote_cmd()
...
function sync_relay.start(local_player_index, auth_role, seed)
    reset_state()
...
function sync_relay.is_active() return active end
function sync_relay.get_error()  return last_error end

-- ── per-tick ─────────────────────────────────────────────────────────────
function sync_relay.apply_pending()
    ...
    if pending_snapshot then
        if is_authority then
            ...
            -- Received from authority. Apply everything authority sends.
            -- Authority is the source of truth for entities (swords, mines, etc.)
            -- so we keep them — they were previously stripped which caused
            -- permanent entity desync between machines.
            ...
            -- Only apply RNG periodically to avoid constant small rewinds.
        end
...
function sync_relay.tick()
    ...
    if last_sent_tick ~= now then
        ...
        send_seq = send_seq + 1
        local_input_history[send_seq] = last_sent_cmd
        local prune_before = send_seq - 240
        if prune_before > 0 then local_input_history[prune_before] = nil end
    ...
    local remote_cmd = resolve_remote_cmd()
...
-- ── per-frame ────────────────────────────────────────────────────────────
function sync_relay.frame()
    if not active or not live then return nil end
    return nil
end

-- ── message handler ──────────────────────────────────────────────────────
function sync_relay.on_message(msg)
    ...
    if t == "remote_input" then
        ...
        local normalized_role, accept = normalize_remote_role(tonumber(msg.role), frame, cmd)
        if not accept then
            return nil
        end
